...
exports.getVendorOrders = void 0;
const database_1 = require("../config/database");
// ─── GET ORDERS RECEIVED ─────────────────────
const getVendorOrders = async (req, res) => {
    try {
        const vendorId = req.vendor.vendorId;
        const [rows] = await database_1.pool.execute(`SELECT p.payment_id, p.transaction_date, p.amount, p.method,
       ...
       WHERE c.vendor_id = ?
       ORDER BY p.transaction_date DESC`, [vendorId]);
        res.json({
            success: true,
            data: rows,
            count: rows.length
